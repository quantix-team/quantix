...
const char * const BITCOIN_CONF_FILENAME = "quantix.conf";
const char * const BITCOIN_PID_FILENAME = "quantixd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "quantix" is a composite category enabling all Quantix-related debug output
            if(ptrCategory->count(string("quantix"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\QuantixCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\QuantixCore
    // Mac: ~/Library/Application Support/QuantixCore
    // Unix: ~/.quantixcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "QuantixCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/QuantixCore";
#else
    // Unix
    return pathRet / ".quantixcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty quantix.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override quantix.conf
